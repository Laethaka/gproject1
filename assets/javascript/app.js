...
$('#add-city-button').on('click', function(event) {
    ...
    $.ajax({
        url: `https://maps.googleapis.com/maps/api/geocode/json?address=${cityInput}&key=`,
        method: 'GET'
...
function initMap(centerLat, centerLng) {
    ...
    var queryURL = `https://cors-anywhere.herokuapp.com/https://maps.googleapis.com/maps/api/place/nearbysearch/json?key=&radius=${distanceInput}&keyword=${activityInput}&location=${centerLat},${centerLng}`
    $.ajax({
        url: queryURL,
        method: 'GET'
    }).then(function(response) {
        ...
        for (let idx=0; idx<sortedArr.length; idx++){
            ...
            markerArr[idx].addListener('click', function() {
                ...
                $.ajax({
                    method: 'GET',
                    url: `https://cors-anywhere.herokuapp.com/https://maps.googleapis.com/maps/api/place/details/json?placeid=${this.placeId}&key=`
                    ...
                    for (var photoIdx = 0; photoIdx < 10; photoIdx++) {
                        ...
                        var photoQueryURL = `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&maxheight=400&photoreference=${photoReference}&key=`;
